...
#         tm_prices = re.findall('<span class="tm-price">(.*?)</span>', res,re.S)
#
#
#         # 30-42行为抓取淘宝商品真实价格，该数据是动态加载的
#         # purl = "https://detail.tmall.com/item.htm?spm={}&id={}&skuId={}&standard=1&user_id={}&cat_id={}&is_b=1&rn={}".format(goods_id)
...
#
#         #特点说明
#         # attributes_html = re.compile('<div id="attributes" class="attributes">(.+?)<div>')
#         # attributes = re.compile('<div id="attributes" class="attributes">.*?</div>', res)
#         attributes =re.findall('<ul id="J_AttrUL">(.+?)</ul>', res, re.S)[0]
#         # attributes1=urllib.request.urlopen(attributes).read().decode('gbk', 'ignore')
#         attributes1 =re.findall('<li (.+?)</li>',attributes)
#          len(attributes1)
#         if i < a
#             attributes2[i] = re.findall('<li (.+?)</li>', attributes1[i])
#             print(attributes2[i])
#
#
#
...
#     url = 'https://detail.tmall.com/item.htm?spm=a220m.1000858.1000725.6.2a5a7040MCDYOi&id=529408043334&skuId=3154155972259&standard=1&user_id=1714128138&cat_id=50918010&is_b=1&rn=3fec71e43d00bacc5a47834d4494d206'
#     # url = input("请输入商品链接: ")
#     spider_tianmao(url)
#
